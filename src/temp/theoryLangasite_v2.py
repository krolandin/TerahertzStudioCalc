...
class TheoryLangasite_v2(Theory):
    name = "Theory Ho langasite v2"
            ...
            for i in range(6):
                for iFi in range(-self.oneSidePointsNum, self.oneSidePointsNum):
                    for iTeta in range(-self.oneSidePointsNum, self.oneSidePointsNum):
                        vectM = self.getVectM(i, iTeta * self.dTeta, iFi * self.dFi)
                        DPos = self.getDPos(vectM)
                        f0 = DPos
                        dMu = self.getDMu(DPos, vectM) * self.normal(iTeta * self.dTeta, self.sigmaTeta) * self.normal(iFi * self.dFi, self.sigmaFi)
    ...
    def calcConstants(self):
        ...
        self.sigmaTeta = self.PI * 30 / 180
        self.sigmaFi = self.PI * 30 / 180
        self.dTeta = 2 * 3 * self.sigmaTeta / (2 * self.oneSidePointsNum + 1)
        self.dFi = 2 * 3 * self.sigmaFi / (2 * self.oneSidePointsNum + 1)
        self.nPos4PI = 4 * self.PI * self.ro / 6 * (3 * self.cc * self.NA / self.MvHoLang) * self.dTeta * self.dFi
    ...
    def normal(self, x, sigma):
        return np.exp(- 0.5 * (x/sigma) ** 2) / np.sqrt(2 * self.PI) / sigma

    def getDPos(self, vectM):  # cm^-1
        return 2 * np.sqrt(self.Dcf ** 2 + (1 / self.kcm * np.dot(self.vectH, vectM)) ** 2)
    ...
    def getDMu(self, DPos, vectM):
        m = vectM[self.hAxisIndex]
        dMuPos = self.nPos4PI * m ** 2 / DPos / self.kcm * np.tanh(DPos * self.kcm / 2 / self.kB / self.T) * (2 * self.Dcf / DPos) ** 2
        return dMuPos
