...
class TheoryLangasite_GaussDirection(Theory):
    name = "Ho LGS GaussDirection"
    ...
    def __init__(self):
        ...
        # for k in range(6):
        #     self.curves.append(TheoryCurve(self.H, self.f_H_6m[k], DataTypes.f_H, "pos " + str(k)))
        #     self.curves.append(TheoryCurve(self.H, self.dMu_H_6m[k], DataTypes.dMu_H, "pos " + str(k)))

        self.curves.append(TheoryCurve(self.H, None, DataTypes.M_H))
    ...
    def calcData_H(self):
        H = np.array([0 + i * (Hmax - 0) / numPoints for i in range(numPoints)], dtype=np.float32)
        eps_H0 = 0
        ...
        # self.f_H_6m = [[] for i in range(6)]
        # self.dMu_H_6m = [[] for i in range(6)]
        # for j in range(numPoints):
        #     self.vectH = np.array([H[j], 0, 0])
        #     for i in range(6):
        #         vectM = self.getVectM(i, 0, 0)
        #         DPos = self.getDPos(vectM, Dcf0)
        #         self.f_H_6m[i].append(DPos)
        #         dMu = self.getDMu_i_dMuH(DPos, vectM)
        #         self.dMu_H_6m[i].append(dMu)

        # gamma = self.gamma.value
        # gamma = 0.1
        mu = calcDmu_H_f(H, f, gamma)
        ...
        m_H = calcM_H(H)
        self.updateCurvePoints(H, m_H, DataTypes.M_H)

        # for k in range(6):
        #     self.updateCurvePoints(H, self.f_H_6m[k], DataTypes.f_H, "pos " + str(k))
        #     self.updateCurvePoints(H, self.dMu_H_6m[k], DataTypes.dMu_H, "pos " + str(k))
    ...
    def getDPos(self, vectM, _Dcf):  # cm^-1
        return 2 * np.sqrt(_Dcf ** 2 + (1 / kcm * np.dot(self.vectH, vectM)) ** 2)
    ...
    def getVectM(self, i, deltaTeta, deltaFi):
        ...
        vectM = mIon * np.array([np.cos(fi) * np.sin(teta), np.sin(fi) * np.sin(teta), np.cos(teta)])
        return vectM

    def getDMu_i_dMuH(self, DPos, vectM):
        m = vectM[axis_h]
        dMuPos = 4 * PI * m ** 2 / DPos / kcm * np.tanh(DPos * kcm / 2 / kB / T) * (
                2 * Dcf0 / DPos) ** 2
        return dMuPos
...
tetaIon = PI * 60 / 180  # math.acos(mc / mIon)  # PI * 58.16 / 180
fiIon = PI * 60 / 180  # math.atan(mb / ma)  # PI * 63.43 / 180
pi23 = 2 * PI / 3

sigmaTeta = PI * 18 / 180  # 8
sigmaFi = PI * 24 / 180  # 14
...
def getDMuPos(DPos, m, _Dcf):
    dMuPos = nPos4PI * m ** 2 / (DPos * kcm) * math.tanh(DPos * kcm / 2 / kB / T) * (2 * _Dcf / DPos) ** 2
    return dMuPos


@jit(float32(float32, float32, float32), nopython=True, nogil=True)
def getDMuPosE(EPos, m, _Dcf):
    dMuPos = nPos4PI * m ** 2 / (2 * EPos * kcm) * math.tanh(EPos * kcm / kB / T) * (_Dcf / EPos) ** 2
    return dMuPos


@vectorize([complex64(float32, float32, float32)], target='parallel')
def calcDmu_H_f(H_i, f_i, gamma):
    mu_i = 1
    for iFi in prange(-oneSidePointsNum, oneSidePointsNum):
        for iTeta in prange(-oneSidePointsNum, oneSidePointsNum):
            # for iDcf in prange(-oneSidePointsNum, oneSidePointsNum):
            for iDcf in prange(0, 2 * oneSidePointsNum):
                for pos in prange(0, 5):
                    ...
                    # DPos = getDPos(Hx, Hy, Hz, vectMx, vectMy, vectMz, float32(Dcf0 + iDcf * dDcf))
                    # DPos = getDPos(Hx, Hy, Hz, vectMx, vectMy, vectMz, float32(iDcf * dDcf2 * 0.5))
                    # if DPos == 0:
                    #     DPos = 0.0001
                    # f0 = DPos
                    ...
                    # dMu = getDMuPos(DPos, vectM, Dcf0 + iDcf * dDcf) * \
                    #       normal(iTeta * dTeta, sigmaTeta) * \
                    #       normal(iFi * dFi, sigmaFi) * \
                    #       normal(iDcf * dDcf, sigmaDcf)

                    # dMu = getDMuPos(DPos, vectM, iDcf * dDcf2 * 0.5) * \
                    #       normal(iTeta * dTeta, sigmaTeta) * \
                    #       normal(iFi * dFi, sigmaFi) * \
                    #       normalX(iDcf * dDcf2)
        ...
        for iTeta in prange(-oneSidePointsNum, oneSidePointsNum):
            ...
            for iDcf in prange(0, 2 * oneSidePointsNum):
                for pos in prange(6):
                    ...
                    # (1/6)*xDistorted_ab*(mu)^2* tanh(D * kcm / (2 * kB * T)) / D
                    # M :=  (3*x*Na)/(MvTbxLGS(x))*H/(kcm)*2*hi

                    n = 1 / 6 * (3 * cc * NA / MvHoLang) * dTeta * dFi * dDcf2
                    m_i += n * (vectMx ** 2 * Hx) * math.tanh(EPos * kcm / kB / T) / (EPos * kcm) * \
                           normal(iTeta * dTeta, sigmaTeta) * \
                           normal(iFi * dFi, sigmaFi) * \
                           normalX(iDcf * dDcf2)
    return m_i
